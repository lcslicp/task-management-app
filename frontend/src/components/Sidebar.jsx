      ...
      <div id='main-links'>
        ...
        <ul className='space-y-2 flex flex-col mt-16'>
          <li className='flex flex-row justify-between'>
            <a
              href='/dashboard'
              className={`flex items-center justify-start w-full rounded-lg py-2 pl-4 pr-4 -ml-4 ${dashboardSelected ? 'text-black bg-white' : 'text-white bg-transparent'}`}
            >
              <svg className={`w-3 h-3 ${dashboardSelected ? 'fill-black' : 'fill-white'}`} viewBox='0 0 14 14' fill='none' xmlns="http://www.w3.org/2000/svg">
          ...
          <li className='w-full'>
            <button
              type='button'
              className={`flex items-center justify-start rounded-lg w-full py-2 -ml-4 pl-4 hover:cursor-pointer ${sort !== 'newest' ? 'bg-white text-black' : sort === 'newest' && toggleSort ? 'bg-hovergray text-white' : 'bg:transparent text-white hover:bg-hovergray'}`}
              onClick={toggleSortDropdown}
            >
              <svg className={`w-3 h-3  ${sort !== 'newest' ? 'fill-black' : sort === 'newest' && toggleSort ? 'fill-white' : 'fill-white'}`} viewBox="0 0 14 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                {sortItems.map((item) => (
                  ...
                  >
                    <input
                      ...
                      name={item.label}
                      value={item.label}
                      className={'w-4 h-4 text-black bg-transparent border border-gray focus:ring-gray-500 checked:border-white focus:ring-0 rounded-lg cursor-pointer'}
          ...
          <li className='w-full'>
            <button
              type='button'
              className={`flex items-center justify-start rounded-lg w-full py-2 -ml-4 pl-4 hover:cursor-pointer ${priorityFilter.length === 0 && toggleFilter ? 'bg-hovergray text-white' : priorityFilter.length !== 0 ? 'bg-white text-black' : 'bg:transparent text-white hover:bg-hovergray'}`}
              onClick={toggleFilterDropdown}
            >
              <svg className={`w-3 h-3  ${priorityFilter.length === 0 ? 'fill-white' :  'fill-black'}`} viewBox='0 0 16 14' fill='none' xmlns='http://www.w3.org/2000/svg'>
        ...
        <a
            ...
            >
              <img
                src={logoutIcon}
                className='w-3 h-3 text-white transition duration-75'
              />
              <span
                className='flex-1 ml-3 whitespace-nowrap text-sm'
                type='button'
